...
public class MaterialTabbed extends JTabbedPane {
    ...
    public class MaterialTabbedUI extends MetalTabbedPaneUI {
        ...
        protected void paintTabBackground(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w, int h, boolean isSelected) {
            ...
            if (isSelected) {
                // Màu xám nhẹ cho tab được chọn (RGB: 240, 240, 240)
                g2.setColor(new Color(240, 240, 240));
                
                // Vẽ background với góc bo tròn nhẹ (tùy chọn)
                if (tabPlacement == TOP) {
                    g2.fillRoundRect(x, y, w, h - 3, 6, 6);
                } else if (tabPlacement == BOTTOM) {
                    g2.fillRoundRect(x, y + 3, w, h - 3, 6, 6);
                } else if (tabPlacement == LEFT) {
                    g2.fillRoundRect(x, y, w - 3, h, 6, 6);
                } else if (tabPlacement == RIGHT) {
                    g2.fillRoundRect(x + 3, y, w - 3, h, 6, 6);
                }
            } else {
                // Màu trong suốt hoặc màu nền mặc định cho tab không được chọn
                if (tabPane.isOpaque()) {
